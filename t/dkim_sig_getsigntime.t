...
SIG_GETSIGNEDTIME: {
	...
	# No timestamp in the signature
	ok($d->dkim_sig_getsigntime($args) == DKIM_STAT_INVALID);

	ok($d->dkim_sig_hdrsigned({ sig => $sig, hdr => 'From' }) == 1);
	ok($d->dkim_sig_hdrsigned({ sig => $sig, hdr => 'Foo' }) == 0);
